...
// Dirubah
const pengirim = "Elky";
const nomorWa = "6282255396903"; // awalan nomor 0 harus di awalin 62
const textWa = "Hehehee";
const pesan = `Aku selalu jatuh hati denganmu :) harapanku kita bisa sama-sama terus dan memenuhi semua impian dan cita-cita yang di dambakan. Sekali lagi Selamat Ulang tahun yaa, kuat terus dan semangat..`;
...
hilang1.addEventListener("click", () => {
  ...
  Swal.fire({
    ...
    imageHeight: 120,
    title: "Isi nama yaa, Aku jatuh hati nihh :)) ?",
    ...
    preConfirm: () => {
      ...
      if (!login) {
        Swal.showValidationMessage(`Masukin Nama Kamu Yaaaa :(`);
      }
      return { login };
    },
  }).then((result) => {
    ...
    let txt = `Halooo, ${result.value.login} Kamu cantik dan baik hati. ${pesan}`;
